...
export(Texture) var asset setget set_texture
var is_active = false
...
func can_move_down():
	if Globals.inactive.has(Vector2(get_parent().position.x + position.x, get_parent().position.y + position.y + Globals.grid_size)) or get_parent().position.y + position.y == Globals.grid_size * 19:
		inactivate_it()
		return false
	else:
		return true
...
func can_move_left():
	if get_parent().position.x + position.x == 0 or (Globals.inactive.has(Vector2(get_parent().position.x + position.x - Globals.grid_size, get_parent().position.y + position.y))) or not is_active:
		return false
	else:
		return true

func can_move_right():
	if get_parent().position.x + position.x == Globals.grid_size * 9 or (Globals.inactive.has(Vector2(get_parent().position.x + position.x + Globals.grid_size, get_parent().position.y + position.y))) or not is_active:
		return false
	else:
		return true
...
func shift_blocks(blocks):
	for i in blocks:
		Globals.inactive[i].y += Globals.grid_size
		Globals.inactive_blocks[i].position.y += Globals.grid_size
